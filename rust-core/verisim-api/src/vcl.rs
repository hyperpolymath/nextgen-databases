...
//!
//! VCL execution endpoint — accepts VCL text queries, parses them, and
//! routes to the appropriate store operations.
//!
//! This is a lightweight server-side VCL parser that handles the core
//! query operations directly against the octad store. It bridges the gap
//! between the REPL client (which sends raw VCL text) and the REST API
//! (which expects structured JSON requests).
//!
//! ## Supported VCL Statements
//!
...
/// VCL execute request — wraps a raw VCL query string.
#[derive(Debug, Deserialize)]
pub struct VclExecuteRequest {
    /// The VCL query text to parse and execute.
    pub query: String,
}

/// VCL execute response — returns structured results from a query.
#[derive(Debug, Serialize)]
pub struct VclExecuteResponse {
    ...
    /// VCL-UT query path used: "VCL (Slipstream)", "VCL-UT", or "VCL-UT".
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// Execute a VCL query string against the database.
///
...
pub async fn vcl_execute_handler(
    State(state): State<AppState>,
    Json(request): Json<VclExecuteRequest>,
) -> Result<Json<VclExecuteResponse>, ApiError> {
    ...
    let mut result = match tokens[0].to_uppercase().as_str() {
        ...
        other => Err(ApiError::BadRequest(format!(
            "Unknown VCL statement: '{}'. Supported: SELECT, SEARCH, INSERT, DELETE, SHOW, COUNT, EXPLAIN",
            other
        ))),
    }?;
...
async fn validate_with_typell(query: &str) -> Option<(u8, String, Vec<String>)> {
    ...
    let path = body
        ...
        .unwrap_or_else(|| "VCL (Slipstream)".to_string());
...
async fn execute_select(
    ...
    tokens: &[String],
    _raw: &str,
) -> Result<VclExecuteResponse, ApiError> {
