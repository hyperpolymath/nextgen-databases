...
function parse_proof!(c::ParseCursor)
    ...
    if kind == "INTEGRITY"
        ...
        return Main.VCLQuery.ProofIntegrity(id)
    ...
    elseif kind == "CONSISTENCY"
        ...
        return Main.VCLQuery.ProofConsistency(id, scope, threshold, weights)
    ...
    elseif kind == "FRESHNESS"
        ...
        return Main.VCLQuery.ProofFreshness(id, window)
...
function _default_weights(scope::Vector{Symbol})
    ...
    if isempty(pairs)
        return Main.FederationManager.DriftWeights(
            Dict{Tuple{Symbol, Symbol}, Float64}())
    end
    w = 1.0 / length(pairs)
    d = Dict{Tuple{Symbol, Symbol}, Float64}()
    for p in pairs
        d[Main.FederationManager.canonical_pair(p...)] = w
    end
    Main.FederationManager.DriftWeights(d)
end
