 ...
 */
let parseGQL = (input: string): result<string, string> => {
...
let gqlParserTarget: fuzzTarget = (input: string): fuzzInputResult => {
  try {
...
let runGQLFuzz = (~config: fuzzConfig=defaultConfig): array<(string, fuzzResult)> => {
  let targets = [
