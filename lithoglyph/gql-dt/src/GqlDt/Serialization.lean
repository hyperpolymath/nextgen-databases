  ...
  | .object fields =>
      ...
      | some (_, .string "BoundedNat") =>
          ...
          | some (_, .number minF), some (_, .number maxF), some (_, .number valF) =>
              ...
              -- Verify bounds from JSON and construct with proofs
              if h1 : min ≤ val then
                if h2 : val ≤ max then
                  .ok ⟨.boundedNat min max, .boundedNat min max ⟨val, h1, h2⟩⟩
                else
                  .error s!"Value {val} exceeds max bound {max}"
              else
...
-- CBOR types and tags imported from GqlDt.Serialization.Types
...
def fromSQLValue (sqlValue : String) (expectedType : TypeExpr) : Except String (Σ t : TypeExpr, TypedValue t) :=
  ...
  | .boundedNat min max =>
      ...
      | some n =>
          if h1 : min ≤ n then
            if h2 : n ≤ max then
              .ok ⟨.boundedNat min max, .boundedNat min max ⟨n, h1, h2⟩⟩
            else
...
-- SerializationFormat imported from GqlDt.Serialization.Types
