...
pub const LithStatus = enum(i32) {
    ...
    pub fn toMessage(self: LithStatus) []const u8 {
        return switch (self) {
            .ok => "Success",
...
test "LithStatus codes" {
    try std.testing.expect(LithStatus.ok.isOk());
    try std.testing.expect(!LithStatus.invalid_proof.isOk());
    try std.testing.expectEqualStrings("Success", LithStatus.ok.toMessage());
}
