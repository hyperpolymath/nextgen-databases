...
| M8 | Form.Runtime (GQL Engine) | v0.0.4 | ✅ Complete
...
* [x] `spec/gql.adoc` - GQL grammar (EBNF, provenance syntax)
* [x] `spec/gql-dependent-types.md` - FQLdt with Lean 4 integration
* [x] `spec/self-normalizing.adoc` - Self-normalizing database spec
* [x] `spec/cloud-storage.adoc` - Object storage integration
* [x] `spec/gql-philosophy.adoc` - Design philosophy
...
* [x] `docs/API-REFERENCE.adoc` - Form.Bridge FFI, GQL API
...
*Goal*: GQL parser/planner/executor.

=== Deliverables

* [x] GQL parser (`core-factor/gql/gql.factor`) - Full PEG-based parser
...
* ✅ GQL PoC subset fully functional
...
* [x] `INTROSPECT SCHEMA` GQL command - Schema inspection
...
* ✅ FunDep.lean types compatible with GQL-dt
...
* [x] Seam tests (`core-factor/gql/seam-tests.factor`) - Full pipeline validation
* [x] End-to-end test suite - Parser → Planner → Executor → Normalizer
* [x] Performance benchmarks (`core-factor/gql/benchmarks.factor`)
* [x] Migration tests (`normalizer/factor/migration-tests.factor`)
* [x] Pluggable storage backend (`core-factor/gql/storage-backend.factor`)
...
[source]
----
POST   /v1/query              # Execute GQL query
...
type Query {
  ...
  query(gql: String!, provenance: ProvenanceInput): QueryResult!
}

type Mutation {
  createCollection(name: String!, schema: SchemaInput!): Collection!
  execute(gql: String!, provenance: ProvenanceInput!): MutationResult!
...
* ✅ All GQL operations accessible via REST, gRPC, and GraphQL
...
* [ ] Composer package
* [ ] GQL query builder (fluent interface)
* [ ] Provenance handling
