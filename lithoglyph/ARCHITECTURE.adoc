...
pub export fn lith_open(path: [*:0]const u8, path_len: usize) callconv(.C) LithStatus {
...
/// Used by GQL (Factor), FQLdt (Lean 4), Elixir control plane
...
pub export fn lith_register_proof_verifier(
    db: *LithDb,
...
GQL parser, planner, and executor.

==== Responsibilities

* **Parser**: GQL syntax → AST
...
GQL is not SQL. Key differences:
...
* Constraints enforced identically via GQL or direct API
