...
syntax "lith_bounds" : tactic
macro_rules
  | `(tactic| lith_bounds) => `(tactic| first | omega | norm_num | decide)

-- Auto-solve provenance proofs
syntax "lith_prov" : tactic
macro_rules
  | `(tactic| lith_prov) => `(tactic| 
      simp only [Tracked, NonEmptyString]; 
      constructor <;> decide)

-- Auto-solve PROMPT score proofs
syntax "lith_prompt" : tactic
macro_rules
  | `(tactic| lith_prompt) => `(tactic|
...
WITH_PROOF {
  ...
  confidence_in_range: by lith_bounds
}
```
...
WITH_PROOF {
  all_scores_in_bounds: by lith_prompt,
  overall_computed_correctly: by lith_prompt,
  rationale_nonempty: by lith_prov
};
...
ADDED_BY "journalist_alice"
WITH_PROOF {
  weight_in_bounds: by lith_bounds,
  reasoning_nonempty: by lith_prov
};
...
WITH_PROOF {
  ...
  new_confidence_valid: by lith_bounds,
  correction_reason_nonempty: by lith_prov
};
...
WITH_PROOF {
  confidence_in_bounds: by lith_bounds
};
