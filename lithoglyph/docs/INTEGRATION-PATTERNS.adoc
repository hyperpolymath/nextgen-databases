...
This document provides complete integration patterns. Native adapters are planned for Milestone M11. Currently, integrations use the documented patterns with GQL API or HTTP REST interface.
====
...
# Enable search through GQL
enabled = true

# GQL SEARCH operator uses Elasticsearch
...
.Using Elasticsearch Search in GQL
[source,gql]
----
-- GQL with Elasticsearch search
...
# Execute GQL query and get Arrow table
...
lith = "https://lith.example.org/ontology#"
...
[source,sparql]
----
PREFIX lith: <https://lith.example.org/ontology#>
...
WHERE {
    ?evidence a lith:Evidence ;
              dc:title ?title ;
              lith:promptScore ?score ;
