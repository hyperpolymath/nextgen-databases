...
Resources at quantity `+1+` must be consumed exactly once. Covers
database connections, prepared statements, and result cursors. No
resource leaks or double-frees are expressible in well-typed programs.
...
proofs cannot reference runtime values. Erased terms (quantity `+0+`)
are never demanded at runtime.

==== 5. Proof Attachment

Query results carry proof witnesses (e.g., `+NoInjection+`) of
statically verified properties. Proofs are erased at runtime (quantity
`+0+`) but available during type checking for downstream composition.
...
`+typeql-effects+` (effect lattice), `+typeql-session+` (session
protocol), `+typeql-proofs+` (proof combinators). Dependencies declared
in `+depends+`.

==== ReScript Modules

`+Lexer.res+` (tokenization), `+Parser.res+` (recursive descent),
`+Ast.res+` (types), `+Serializer.res+` (AST to JSON). Interface files
