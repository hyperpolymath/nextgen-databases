...
VCL's cross-modal semantics (HEXAD, 8 modalities, PROOF clauses) are the natural
...
natural modal type constraint in VCL but meaningless in SQL).
...
The EBNF extends VCL v3.0 by appending optional clauses AFTER the standard query.
No keyword conflicts. All 6 new clause keywords (`CONSUME`, `SESSION`, `EFFECTS`,
`TRANSACTION`, `ATTACHED`, `USAGE`) are disjoint from VCL's 60+ reserved keywords.
...
* **Phase 4**: Examples & Tests (11 files) — 7 .vclut examples, 3 Idris2 test
...
4. (Future) ReScript parser compiles and accepts all 7 example `.vclut` files
