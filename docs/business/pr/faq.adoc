...
proofs (VCL-UT). The fundamental difference is architectural: Weaviate treats
...
VCL (VeriSim Consonance Language) is VeriSimDB's purpose-built query language
...
=== What are dependent-type proofs (VCL-UT)?

VCL-UT is an optional extension to VCL that adds dependent-type verification.
When enabled, VCL-UT uses the Idris2 type system to prove properties of queries
at compile time:
...
VCL-UT is entirely optional. Teams can use plain VCL for standard operations and
adopt VCL-UT incrementally when they want formal guarantees.
...
* *Medium-term (6-18 months):* Container images, managed service pilot, VCL-UT
