...
This complements link:vcl-formal-semantics.adoc[VCL Formal Semantics] which covers operational semantics.

== Type Language

=== Type Syntax

The VCL type language stem:[\tau] is defined by:
...
|`GRAPH`
|stem:[\text{Octad} \to \text{Set}(\text{RDFTriple})]

|`VECTOR`
|stem:[\Pi n : \text{Nat}. \text{Octad} \to \text{Option}(\text{Vector}[n])]

|`TENSOR`
|stem:[\Pi d_1, \ldots, d_k : \text{Nat}. \text{Octad} \to \text{Option}(\text{Tensor}[d_1, \ldots, d_k])]

|`SEMANTIC`
|stem:[\text{Octad} \to \text{Set}(\text{TypeAnnotation})]

|`DOCUMENT`
|stem:[\text{Octad} \to \text{Option}(\text{String})]

|`TEMPORAL`
|stem:[\text{Octad} \to \text{List}(\text{Version})]
|===
...
|stem:[\Gamma \vdash C : \text{Octad} \to \text{Bool}]
...
\frac{
  \Gamma \vdash \mathcal{M} : \text{ModalitySet} \quad
  \Gamma \vdash S : \text{Source} \quad
  \Gamma \vdash C : \text{Octad} \to \text{Bool} \quad
...
\frac{
  ...
  \Gamma \vdash p : \text{Octad} \to \text{Bool}
} \text{(T-GraphCond)}
++++
...
\frac{
  ...
  \Gamma \vdash \texttt{h.embedding SIMILAR TO } v \texttt{ WITHIN } t : \text{Octad} \to \text{Bool}
...
\frac{
  ...
  \Gamma \vdash \texttt{h.embedding NEAREST } k \texttt{ USING } m : \text{Octad} \to \text{Bool}
} \text{(T-VectorNearest)}
++++
...
++++
\frac{
  \Gamma \vdash \text{field} : \text{Octad} \to \text{Tensor}[\ldots] \quad
  \Gamma \vdash \text{op} : \text{TensorOp} \quad
  \Gamma \vdash \text{literal} : \text{Tensor}[\ldots]
}{
  \Gamma \vdash \text{field op literal} : \text{Octad} \to \text{Bool}
} \text{(T-TensorCond)}
++++
...
\frac{
  ...
  \Gamma \vdash \texttt{SATISFIES } c(p) : \text{Octad} \to \text{Bool}
} \text{(T-SemanticCond)}
++++
...
\frac{
  ...
  \Gamma \vdash \texttt{AS OF } t : \text{Octad} \to \text{Bool}
} \text{(T-TemporalAsOf)}
++++
...
++++
\frac{
  \Gamma \vdash C_1 : \text{Octad} \to \text{Bool} \quad
  \Gamma \vdash C_2 : \text{Octad} \to \text{Bool}
}{
  \Gamma \vdash C_1 \texttt{ AND } C_2 : \text{Octad} \to \text{Bool}
} \text{(T-And)}
++++
...
\frac{
  ...
  \Gamma \vdash C_1 \texttt{ OR } C_2 : \text{Octad} \to \text{Bool}
} \text{(T-Or)}
++++
...
\frac{
  ...
  \Gamma \vdash \texttt{NOT } C : \text{Octad} \to \text{Bool}
} \text{(T-Not)}
++++
...
\frac{
  ...
  \text{Octad}_{\mathcal{M}_2} \leq \text{Octad}_{\mathcal{M}_1}
} \text{(Sub-Octad)}
++++
...
|**Result Type**
|stem:[\text{List}(\text{Octad})]
|stem:[\Sigma r : \text{List}(\text{Octad}). \text{Proof}[\phi(r)]]
...
- link:vcl-formal-semantics.adoc[VCL Formal Semantics]
- link:vcl-grammar.ebnf[VCL Grammar]
