...
struct Store
    ...
    keypair::Any                     # Crypto.Ed25519KeyPair (duck-typed to avoid load order)
...
function ed25519_sign(keypair, payload::Vector{UInt8})::Signature
    sig_bytes = Crypto.sign_detached(keypair, payload)
...
function ed25519_verify(sig::Signature, payload::Vector{UInt8})::Bool
    Crypto.verify_detached(sig.key_id, sig.sig_bytes, payload)
end
