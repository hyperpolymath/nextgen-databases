...
│    ├── QueryRouter     (distributes VCL queries)     │
...
* **Deno** 2.0+ (for VCL parser bridge -- falls back to built-in parser)
...
A **octad** (historical name; now an octad) is an entity with up to 8
...
VeriSimDB uses its own query language, *VCL* (VeriSim Consonance Language),
designed for cross-modal queries. VCL is not SQL -- it natively
...
=== VCL-UT (Formally Verified Queries)

VCL-UT extends VCL with proof certificates. Every result includes a
...
NOTE: VCL-UT proof generation is under active development. The syntax
...
VCL query can transparently route to PostgreSQL, ArangoDB, Elasticsearch,
...
| `POST` | `/api/v1/octads` | Create a new entity
| `GET` | `/api/v1/octads/:id` | Retrieve entity by ID
| `PUT` | `/api/v1/octads/:id` | Update entity
| `DELETE` | `/api/v1/octads/:id` | Delete entity
...
=== VCL parser falls back to built-in

This is normal. The external Deno-based VCL parser provides richer
error messages but the built-in Elixir parser handles all standard
queries. The fallback warning can be safely ignored.

== Next Steps

* Read the link:VCL-SPEC.adoc[VCL Specification] for the full query language
* See link:vcl-examples.adoc[VCL Examples] for more query patterns
