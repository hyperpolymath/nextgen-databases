...
Strategy for maintaining backwards compatibility across VCL versions, schema evolution, and API changes.
...
3. **Test both versions side-by-side:**
   ```elixir
   old_result = VCL.parse_and_execute(old_query, version: "1.0.0")
   new_result = VCL.parse_and_execute(new_query, version: "1.2.0")
...
X-VCL-Version-Supported: 1.0.0, 1.1.0
