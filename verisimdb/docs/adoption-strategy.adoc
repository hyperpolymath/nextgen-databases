...
* *Formally verifiable queries* (VCL-UT) with proof certificates
...
its expression profile. VCL-UT provides reproducibility guarantees. Provenance
...
from the EHR -- a life-safety issue. VCL-UT proof certificates satisfy audit
requirements.
...
[cols="1,1,1,1,1,1"]
|===
| Domain | Drift Detection | Federation | VCL-UT | Readiness | Priority
...
| VCL-UT proofs incomplete | PROOF clauses parse but don't generate verifiable
  certificates yet. Lean type checker integration planned Q2 2026.

| No SQL dialect | VCL is intentionally not SQL. Users wanting familiar syntax
  need to learn VCL (comprehensive docs provided).
