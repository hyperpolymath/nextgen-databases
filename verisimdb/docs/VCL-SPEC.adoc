...
= VeriSim Query Language (VCL) Specification
...
The VeriSim Query Language (VCL) is the native query interface for VeriSimDB, a cross-system entity consistency engine with drift detection, self-normalisation, and formally verified queries. This document is the **normative language specification** for VCL version 2.0.
...
* **Dual-path execution** — Slipstream (fast, unverified) and VCL-DT (formally verified with ZKP proofs).
...
[cols="1,1,1"]
|===
| Property | Slipstream (no PROOF) | VCL-DT (with PROOF)
...
* **Level 2 — Full (Slipstream + VCL-DT)**: All Level 1 features plus PROOF clause parsing, dependent type checking, proof generation, and proof certificate bundling.
...
VCL has a rich type system that operates in two modes depending on the execution path. The slipstream path uses simple types for basic validation. The VCL-DT path adds dependent types, refinement types, and proof types for formal verification.
...
The PROOF clause activates the VCL-DT execution path and specifies proof obligations.
...
When a VCL-DT query succeeds, the result includes a **proof certificate**:
...
| VCL-DT runtime (Lean type checker wired to execution)
| Next milestone
...
[source,ebnf]
----
(* VeriSim Query Language (VCL) Grammar *)
...
| Detailed comparison of Slipstream vs VCL-DT execution paths, six proof types, performance implications, honest implementation status. 282 lines.
...
// ============================================================================
// APPENDIX E: VCL-dt++ EXTENSIONS (TYPELL INTEGRATION)
// ============================================================================

[appendix]
== VCL-dt++ Extensions (Typell Integration)

VCL-dt++ extends VCL v3.0 with six optional clauses that provide maximal type-theoretic strictness. These extensions are consumed by the **Typell verification kernel** (the formal verification substrate for PanLL) and are specified normatively in the grammar delta file:
...
[cols="2,1,1,2"]
|===
| Feature | VCL-dt | VCL-dt++ | Typell Kernel Mapping
...
Each dt++ clause is shown in isolation to demonstrate independent usage.
...
VCL-dt++ queries are verified by the Typell kernel via JSON-RPC:

[source]
----
typell.check(query, "vcl-dt++") → TypeResult
----
...
`[.planned]` All six VCL-dt++ clauses. Grammar formally specified. Implementation pending Typell kernel (Phase 3+).
