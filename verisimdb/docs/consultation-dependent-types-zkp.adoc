...
WHERE octad.types INCLUDES "Paper"
  AND VERIFIED(octad.document.peer_reviewed = true)
PROOF EXISTENCE {
...
SELECT GRAPH
FROM verisim:semantic
WHERE octad.graph.citation_count > 10
      AND VERIFIED(octad.graph.peer_reviewed = true)

-- Type checking steps:
-- 1. Synthesize: octad.graph.citation_count → Int
...
WHERE octad.types INCLUDES "MedicalRecord"
PROOF ACCESS {
...
fn compute_witness(
    ...
    match contract.contract_type {
        ContractType::Universal => {
            ...
            for octad in &result.octads {
                let satisfied = evaluate_predicate(&contract.predicate, octad)?;
...
SELECT GRAPH
FROM verisim:semantic
WHERE VERIFIED(octad.graph.node_count > "foo")  -- Type error
PROOF EXISTENCE { ... };
```
...
let t1 = synthesize ctx (EField (EVar "octad", "graph", "node_count")) in
...
pub fn type_check_cached(query: &VCLQuery) -> Result<DependentType, TypeError> {
...
pub async fn generate_proofs_batch(
    queries: Vec<(VCLQuery, QueryResult)>,
...
```rust
pub fn execute_query_erased(query: &VCLQuery) -> Result<QueryResult, VCLError> {
...
pub extern "C" fn type_check_query(
    ...
    let query: VCLQuery = serde_json::from_str(query_str).unwrap();
...
let checkQuery = (query: VCLQuery.t): result<DependentType.t, TypeError.t> => {
  let queryJson = query->VCLQuery.toJson->Js.Json.stringify
...
```rescript
// VCLTypeChecker.res
let isVerifiable = (expr: VCLExpr.t): bool => {
  switch expr {
  | EBinOp(Eq, _, _) => true
...
See docs/vcl-type-system.adoc for full 30+ typing rules.
