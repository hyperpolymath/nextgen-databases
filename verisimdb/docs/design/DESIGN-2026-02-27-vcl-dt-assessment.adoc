...
VCL-DT has *substantially more real implementation than initially expected*.
...
**Honest overall VCL-DT completion: ~35%** (individual components exist;
...
| *FULLY IMPLEMENTED.* Parses sanctify-php security reports, security issue
  ...
  Tangential to VCL-DT but functional.
|===
...
| *IMPLEMENTED.* Routes queries by type (:text, :vector, :graph, :semantic,
  :temporal, :multi) to RustClient. No VCL-DT-specific logic.
|===
...
2. Does **not type-check VCL-DT queries** (only the ReScript layer does this,
   client-side)
...
The CLAUDE.md Known Issues section acknowledges: "VCL-DT not connected to VCL
...
1. Define VCL-DT's type system in Lean 4 or Idris2.
...
do not talk to each other for the VCL-DT path.
...
3. **The shortest path to a working VCL-DT pipeline** is wiring the ReScript
...
5. **The Lean type checker is aspirational.** The ReScript type checker is
   capable enough for practical VCL-DT use. A Lean version would provide
