...
=== Layer 3: VCL-DT — Formally Verified Queries (Technical Moat)
...
No database in existence does this.  VCL-DT is the reason a competitor
...
**VCL-DT is not the lead pitch.** It is the moat.  Drift detection gets
people in the door.  Federation keeps them.  VCL-DT ensures nobody can
compete.
...
**Scope:** Not VCL-DT.  Just VCL.  The query language that end users type.
...
=== Priority 7: Wire VCL-DT (HIGH — but after VCL)

**Why:** VCL-DT is the technical moat — the reason nobody can clone VerisimDB
...
VCL-DT is not a nice-to-have.  It is the capability that separates VerisimDB
from every other database and every data quality tool.  Great Expectations can
assert "this column has no nulls."  VerisimDB with VCL-DT can return a
...
7. **Wire VCL-DT** so the technical moat is _operational_ (after VCL is solid)
...
- **VCL-DT** ensures nobody can compete (years of type theory to replicate)
