...
| **Self-normalisation**
| Real implementation.  Regeneration strategies inspect octad data, select the
...
| **Temporal versioning**
| Real implementation.  Native version history per octad, not audit table hacks.
...
| **VCL is incomplete**
| HIGH
| VCL parses and executes basic queries, but the language is not fully specified
...
| The stores wrap Oxigraph, Tantivy, HNSW, ndarray, CBOR.  This is fine
  architecturally -- the novel contribution is the octad coordination and drift
...
=== Layer 3: VCL-UT — Formally Verified Queries (Technical Moat)
...
No database in existence does this.  VCL-UT is the reason a competitor
...
**VCL-UT is not the lead pitch.** It is the moat.  Drift detection gets
people in the door.  Federation keeps them.  VCL-UT ensures nobody can
compete.
...
== From Octad to Octad: Two New Modalities
...
- Query: `SELECT octads WHERE provenance.origin = 'postgres'` or
...
- VCL extension: `PROVENANCE` clause alongside existing `PROOF` clause

==== Priority

**CRITICAL.**  Implement after VCL (Priority 1) and the drift demo
...
**Scope:** Not VCL-UT.  Just VCL.  The query language that end users type.

**What "finished" means:**

1. **Language specification** -- a document (adoc or djot) that defines VCL
...
2. **Cross-modal queries work end-to-end** -- a query like
   `SELECT octads WHERE drift_score > 0.3 AND modality.temporal.version > 5`
...
4. **Aggregates work** -- `COUNT`, `AVG`, `MIN`, `MAX`, `SUM` over octad
...
- `docs/VCL-SPEC.adoc` -- language specification
...
- Do not wait for VCL to be perfect; the REST API is sufficient for a first
  user
...
4. **Tutorial** -- a 10-minute "store a octad, search it, check its drift,
...
1. **Adapter interface** — a trait/behaviour that maps an external database's
   API to VerisimDB's octad model.  Each adapter knows how to read an entity
...
=== Priority 7: Wire VCL-UT (HIGH — but after VCL)

**Why:** VCL-UT is the technical moat — the reason nobody can clone VerisimDB
quickly.  Formally verified query results via Lean type checking is genuinely
unique in the database world.  But VCL must work reliably first (Priority 1).
No point verifying proofs in a query language that cannot execute reliably.

**Why it matters more than it might seem:**

VCL-UT is not a nice-to-have.  It is the capability that separates VerisimDB
from every other database and every data quality tool.  Great Expectations can
assert "this column has no nulls."  VerisimDB with VCL-UT can return a
...
**Defer until:** VCL spec is complete and integration-tested (Priority 1).
Then this becomes the highest priority.

**See:** KNOWN-ISSUES #21, `docs/vcl-vs-vcl-dt.adoc` for the phased roadmap.
...
1. **Finish VCL** so the database is _queryable_
...
7. **Wire VCL-UT** so the technical moat is _operational_ (after VCL is solid)
...
- **VCL-UT** ensures nobody can compete (years of type theory to replicate)
