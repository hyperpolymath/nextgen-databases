...
=== Example 1: Simple Octad Retrieval
...
**Combining aggregates with formal verification (VCL-UT path):**
[source,vcl]
----
SELECT DOCUMENT.name, COUNT(*)
FROM FEDERATION /universities/* WITH DRIFT STRICT
...
field against the graph centrality field on the same octad.
...
Returns octads where the vector embedding has drifted more than 0.3 from the
...
Returns octads that have vector embeddings but lack tensor data. Useful for
identifying octads that need tensor representations generated.
...
Creates a new octad with document and vector modalities populated.
The octad ID is auto-generated and returned in the result.
...
Creates a octad with document content, embedding, and a graph edge
(CITES relationship to an existing octad).
...
=== Example 60: Update Octad Fields
...
Removes the octad and all its modality data. This operation is logged
