...
This document specifies the **formal operational semantics** and **static type system** for VeriSim Consonance Language (VCL). VCL has two execution paths with different semantic guarantees:
...
We define the abstract syntax of VCL (concrete syntax in link:vcl-grammar.ebnf[vcl-grammar.ebnf]):
...
VCL has the following base types:
...
Where stem:[\text{Valid}(h, m)] asserts that octad `h` has a valid representation in modality `m`.
...
\frac{
  ...
  \Gamma \vdash \text{condition} : \text{Octad} \to \text{Bool}
}{
...
\frac{
  \Gamma \vdash \text{modalities} : \mathcal{M} \quad
  \Gamma \vdash \text{source} : \text{Source} \quad
  \Gamma \vdash \text{condition} : \text{Octad} \to \text{Bool} \quad
...
\frac{
  ...
  \Gamma \vdash \texttt{WHERE } \text{pattern} : \text{Octad} \to \text{Bool}
}
++++
...
\frac{
  ...
  \Gamma \vdash \texttt{WHERE h.embedding SIMILAR TO } v \texttt{ WITHIN } \text{threshold} : \text{Octad} \to \text{Bool}
}
++++
...
1. **Store Environment** stem:[\Sigma]: Maps UUIDs to Octad data
2. **Proof Environment** stem:[\Pi]: Maps contracts to proof functions

[stem]
++++
\begin{aligned}
\Sigma &: \text{UUID} \rightharpoonup \text{OctadData} \\
\Pi &: \text{Contract} \rightharpoonup (\text{OctadData} \to \text{Proof})
\end{aligned}
++++
...
**Interpretation:**
1. Retrieve candidate octads from source
...
++++
\frac{
  \Sigma(h) = \text{octad-data} \quad
  \text{modality} \in \mathcal{M}
}{
  \text{project}_\mathcal{M}(h)[\text{modality}] = \text{extract}(\text{octad-data}, \text{modality})
}
++++
...
For dependent-type queries, VCL delegates to **proven-library** (external ZKP framework):
...
Where stem:[\mathcal{P}(\text{Octad})] is the powerset of octads.
...
- link:vcl-grammar.ebnf[VCL Grammar (ISO EBNF)]
- link:vcl-type-system.adoc[VCL Type System Specification]
