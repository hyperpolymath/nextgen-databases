...
We describe VCL-DT, a query language extension that supports proof-carrying queries with 11 formally verified proof types, enabling clients to request cryptographic certificates attesting to the consistency, provenance, and integrity of query results.
...
4. **VCL-DT** -- a dependent-type extension to the VeriSim Query Language (VCL) that supports 11 proof types, enabling clients to request and verify formal certificates of data consistency, integrity, and provenance.
...
VCL-DT draws inspiration from proof-carrying code <<necula1997>> and dependent type systems in languages such as Idris 2 <<brady2021>> and Lean 4 <<demoura2021>>, adapting these ideas to the database query context.
...
The VeriSim Query Language (VCL) is the native query interface for VeriSimDB.
...
VCL-DT (VCL with Dependent Types) is activated by the presence of a `PROOF` clause.
...
When a VCL-DT query is executed, the proof pipeline generates a _proof obligation_ for each requested proof type.
...
VCL-DT supports three composition strategies for multi-proof queries:
...
- **VeriSim.QueryRouter**: Routes VCL queries to the appropriate execution pipeline (slipstream vs VCL-DT) and handles federation decomposition.
...
| VCL-DT with INTEGRITY proof
| 8.3
| 27.4
|===

VCL-DT queries incur approximately 2-3x the latency of slipstream queries due to proof generation and certificate construction.
...
VCL-DT is, to our knowledge, the first query language to support proof-carrying results with dependent-type semantics.
...
**Lean 4 integration for VCL-DT type checking.**
The current VCL-DT type checker is implemented in Elixir with a bridge to the Rust ZKP module.
We plan to integrate Lean 4 as an external proof assistant, enabling VCL-DT contracts to be expressed as Lean theorems and verified by Lean's kernel.
...
VCL-DT extends this consistency guarantee to the query interface, enabling clients to not merely _hope_ that their data is consistent but to _prove_ it with cryptographic certificates.
