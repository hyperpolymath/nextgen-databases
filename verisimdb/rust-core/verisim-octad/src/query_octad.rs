...
mod tests {
    ...
    #[test]
    fn test_build_query_octad() {
        let (id, input) = QueryOctadBuilder::new("SELECT * FROM octads WHERE drift > 0.5")
            .with_id("query-001")
    ...
    fn test_auto_id_generation() {
        let (id, _input) = QueryOctadBuilder::new("SELECT 1").build();
        assert!(id.0.starts_with("query-"));
    }

    #[test]
    fn test_minimal_query_octad() {
        let (_, input) = QueryOctadBuilder::new("REFLECT").build();
        assert!(input.document.is_some());
