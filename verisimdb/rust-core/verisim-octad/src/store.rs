...
//! In-memory OctadStore implementation
//!
...
pub struct OctadSnapshot {
    pub id: OctadId,
    pub input: OctadInput,
    pub modality_status: ModalityStatus,
    pub timestamp: DateTime<Utc>,
}

/// In-memory implementation of OctadStore
///
...
pub struct InMemoryOctadStore<G, V, D, T, S, R, P, L>
where
    G: GraphStore,
    V: VectorStore,
    D: DocumentStore,
    T: TensorStore,
    S: SemanticStore,
    R: TemporalStore<Data = OctadSnapshot>,
    P: ProvenanceStore,
    L: SpatialStore,
{
    config: OctadConfig,
    /// Octad status registry
    octads: Arc<RwLock<HashMap<String, OctadStatus>>>,
...
impl<G, V, D, T, S, R, P, L> InMemoryOctadStore<G, V, D, T, S, R, P, L>
where
    G: GraphStore,
    V: VectorStore,
    D: DocumentStore,
    ...
    L: SpatialStore,
{
    /// Create a new in-memory octad store with all eight modality stores.
    ///
    ...
    pub fn new(
        ...
        Self {
            config,
            octads: Arc::new(RwLock::new(HashMap::new())),
    ...
    async fn wal_checkpoint(&self) -> Result<(), OctadError> {
        if let Some(ref wal) = self.wal {
            let mut writer = wal.lock().await;
            writer.checkpoint().map_err(|e| OctadError::ModalityError {
                modality: "wal".to_string(),
    ...
    async fn process_graph(
        &self,
        id: &OctadId,
        input: &OctadGraphInput,
    ) -> Result<GraphNode, OctadError> {
    ...
    /// Process vector input for a octad
    async fn process_vector(
        &self,
        id: &OctadId,
        input: &OctadVectorInput,
    ) -> Result<Embedding, OctadError> {
        if input.embedding.len() != self.config.vector_dimension {
            return Err(OctadError::ValidationError(format!(
    ...
    /// Process document input for a octad
    ...
    /// Process tensor input for a octad
    ...
    /// Process semantic input for a octad
    async fn process_semantic(
        &self,
        id: &OctadId,
        input: &OctadSemanticInput,
    ) -> Result<SemanticAnnotation, OctadError> {
    ...
    /// Process provenance input for a octad — records a lineage event
    async fn process_provenance(
        &self,
        id: &OctadId,
        input: &OctadProvenanceInput,
    ) -> Result<u64, OctadError> {
    ...
    /// Process spatial input for a octad — indexes geospatial data
    async fn process_spatial(
        &self,
        id: &OctadId,
        input: &OctadSpatialInput,
    ) -> Result<SpatialData, OctadError> {
        let coordinates = Coordinates::new(input.latitude, input.longitude, input.altitude)
            .map_err(|e| OctadError::ValidationError(e.to_string()))?;
    ...
    async fn rollback_create(&self, id: &OctadId, written_modalities: &ModalityStatus) {
    ...
    /// Load a complete Octad from all stores
    async fn load_octad(&self, id: &OctadId) -> Result<Option<Octad>, OctadError> {
        let octads = self.octads.read().await;
        let status = match octads.get(id.as_str()) {
            Some(s) => s.clone(),
            None => return Ok(None),
        };
        drop(octads);
...
#[async_trait]
impl<G, V, D, T, S, R, P, L> OctadStore for InMemoryOctadStore<G, V, D, T, S, R, P, L>
where
    G: GraphStore + 'static,
    ...
    L: SpatialStore + 'static,
{
    ...
    async fn create(&self, input: OctadInput) -> Result<Octad, OctadError> {
        ...
        for modality in &modality_names {
                ...
                .await
            {
                ...
                return Err(OctadError::ConsistencyViolation(format!(
    ...
    async fn status(&self, id: &OctadId) -> Result<Option<OctadStatus>, OctadError> {
        Ok(self.octads.read().await.get(id.as_str()).cloned())
    }
        ...
        for edge in edges {
            if edge.predicate.iri == predicate_iri {
                if let GraphObject::Node(target) = edge.object {
                    ...
                    if let Some(octad) = self.load_octad(&OctadId::new(target_id)).await? {
                        octads.push(octad);
                    }
                }
            }
    ...
    async fn list(&self, limit: usize, offset: usize) -> Result<Vec<Octad>, OctadError> {
        ...
        for id_str in ids {
            if let Some(octad) = self.load_octad(&OctadId::new(&id_str)).await? {
                result.push(octad);
            }
        }
        Ok(result)
