...
defmodule VeriSim.Query.VCLTest do
  @moduledoc """
  VCL end-to-end tests for parsing, routing, and execution.

  Tests the VCL Slipstream path (no PROOF clause) across all 8 octad modalities.
  Uses the built-in Elixir fallback parser via VCLBridge.parse/1.
  """
  ...
  setup_all do
    # Start VCLBridge GenServer (will fall back to built-in parser without Deno)
  ...
  describe "VCLBridge.parse/1 — SELECT queries" do
    ...
    test "parses single-modality VECTOR query" do
      {:ok, ast} = VCLBridge.parse("SELECT VECTOR.* FROM HEXAD 'abc-123'")
      assert is_map(ast)
    end

    test "parses single-modality DOCUMENT query" do
      {:ok, ast} = VCLBridge.parse("SELECT DOCUMENT.* FROM HEXAD 'abc-123'")
      assert is_map(ast)
    end

    test "parses single-modality TEMPORAL query" do
      {:ok, ast} = VCLBridge.parse("SELECT TEMPORAL.* FROM HEXAD 'abc-123'")
      assert is_map(ast)
    end

    test "parses single-modality PROVENANCE query" do
      {:ok, ast} = VCLBridge.parse("SELECT PROVENANCE.* FROM HEXAD 'abc-123'")
      assert is_map(ast)
    end

    test "parses single-modality SPATIAL query" do
      {:ok, ast} = VCLBridge.parse("SELECT SPATIAL.* FROM HEXAD 'abc-123'")
      assert is_map(ast)
    end

    test "parses multi-modality query" do
      {:ok, ast} =
        VCLBridge.parse("SELECT GRAPH.*, VECTOR.*, DOCUMENT.* FROM HEXAD 'abc-123'")

      assert is_map(ast)
    end

    test "parses ALL modality wildcard" do
      {:ok, ast} = VCLBridge.parse("SELECT * FROM HEXAD 'abc-123'")
      assert is_map(ast)
    end
  ...
  describe "VCLBridge — mutations via parse_statement/1" do
    test "parses INSERT with document data" do
      {:ok, ast} =
        VCLBridge.parse_statement(
    ...
    test "parses DELETE" do
      {:ok, ast} =
        VCLBridge.parse_statement("DELETE HEXAD 'abc-123'")

      assert is_map(ast)
    end
  end

  describe "VCLBridge.parse/1 — error cases" do
    test "rejects empty string" do
      assert {:error, _} = VCLBridge.parse("")
    end

    test "rejects gibberish" do
      assert {:error, _} = VCLBridge.parse("THIS IS NOT VCL AT ALL")
    end
  end
  ...
  describe "VCLExecutor.execute_string/2" do
    test "returns parse error for invalid query" do
      assert {:error, {:parse_error, _}} = VCLExecutor.execute_string("NOT VCL")
    end
  ...
  describe "VCLExecutor.execute_statement/2" do
    test "routes query AST through execute without crashing" do
      {:ok, ast} = VCLBridge.parse_statement("SELECT GRAPH.* FROM HEXAD 'abc-123'")
      ...
      result =
        try do
          VCLExecutor.execute_statement(ast, timeout: 1_000)
        rescue
    ...
    test "routes mutation AST through execute_mutation without crashing" do
      {:ok, ast} = VCLBridge.parse_statement("DELETE HEXAD 'abc-123'")
  ...
  describe "provenance query routing" do
    test "parses provenance-only query" do
      {:ok, ast} =
        VCLBridge.parse("SELECT PROVENANCE.* FROM HEXAD 'abc-123'")
  ...
  describe "spatial query routing" do
    test "parses spatial-only query" do
      {:ok, ast} =
        VCLBridge.parse("SELECT SPATIAL.* FROM HEXAD 'abc-123'")
