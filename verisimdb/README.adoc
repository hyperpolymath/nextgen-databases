...
VCL provides unified querying across all eight modalities with two execution paths:
...
=== VCL-UT (Dependent Types with Proof Certificates)
...
VCL-UT queries return a `ProvedResult` with data AND a proof certificate:
...
ZKP proofs and dependent types enable tamper-evident knowledge exchange. VCL-UT queries return data with cryptographic proof certificates — you can verify the result without trusting the source.

== Documentation

* link:docs/vcl-grammar.ebnf[VCL Grammar] — Formal EBNF specification
* link:docs/vcl-formal-semantics.adoc[VCL Formal Semantics] — Operational semantics
* link:docs/vcl-type-system.adoc[VCL Type System] — Dependent types and bidirectional type checking
